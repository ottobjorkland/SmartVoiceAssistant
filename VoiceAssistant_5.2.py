...
from CustomSettings import sweOverrideMemPrompt, GPT4
...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = os.path.join(scriptDir, GOOGLE_JSON_CREDENTIALS)
    ...
    if keyword_index >= 0: # If wake-word detected
        ...
        while time.time() - start_time < listenTime: # Listen for certain amount of seconds
            ...
            if userPrompt is not None or userPrompt != '':
                ...
                '''if (("summarize" in userPrompt) and ("history" in userPrompt)) or ("summarizeHistory" in userPrompt):
                    summarizeHistory()
                    userPrompt = None'''
...
def useTool(userPrompt):
    ...
    toolStartPrompt = (
            ...
            #+'Choose a tool now within {}, and write a reason and cite the text that made your choice.' # TEST
            )
    ...
    #response1=generateResponse(toolMsgs, temp=0.2) # TEST

    global devMode
    if devMode: print("response1:",response1) # TEST
    ...
    while useTools:
        ...
        if 'RESET MEMORY' in response1:
            ...
            if rstMemStage == 2: # Did the user approve?
                ...
                #rstMemMsgs.append({'role':'system','content':memRstStartPrompt})
                ...
                '''match2 = re.search(r'\{(.+?)\}', memRstChoice) # Extract the response in braces
                if match2: memRstChoice = match2.group(1)
                elif 'LONG TERM' not in memRstChoice or 'SHORT TERM' not in memRstChoice or 'BOTH' not in memRstChoice:
                    memRstChoice = "DON'T KNOW"'''
        ...
        elif 'GOOGLE SEARCH' in response1:
    ...
    if language == "sv": # Swedish
        ...
        try:
            if vlcLib == True: # For Raspberry Pi
                ...
                '''if response.status_code != 200: # There was an error
                    print(Style.BRIGHT+Fore.RED+"ElevenLabs Error:")
                    print(Style.BRIGHT+Fore.RED+response.text)
...
def generateResponse(openAImsgs, funcList=None, temp=None, functionCall=None, stream=None, max_tokens=None):
    ...
    if promptTokens > 3500:
        if GPT4 == False: print(Fore.RED+"OpenAI: Using 16K model instead of 4K, which is more expensive (too many tokens for 4K model)")
        else: print(Fore.RED+"OpenAI: Using GPT-3.5-Turbo-16K model instead of GPT-4 with 4K limit (too many tokens for 4K model)")
        if funcList is not None:
    ...
    '''
    # This can be used sometime if it is needed for counting tokens
    if stream is None: # If response isn't streamed
        global messageTokens
        messageTokens = response['usage']['total_tokens']
        print(f"messageTokens: {messageTokens}")
    '''
...
def summarizeHistory(userPrompt=None):
    ...
    # WARNING! longTermMemory.json can only contain one list

    # Extract previous summations from the JSON file and append them to memoryList
    if "summations" in data: # If there are any summations
        oldSummation = data["summations"][0] # TEST
        '''for summations in data["summations"]:
            oldSummation = summations'''
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            #messages.append({"role": "user", "content": summarizePrompt}) # TEST
            ...
            if ('{none}' not in newSummation) and ("User's name" in newSummation):
                ...
                for name in range(len(newSummationList)): # Kör en gång för varje namn i nya summeringen
                    newName=newSummationList[name]["User's name"]
                    newInfo=newSummationList[name]["Info"]

                    # Om namnet från den nya summeringen redan finns med i gamla summeringen
                    if "summations" in data:
                        if (newName in oldNameList): # Kolla om något av den nya infon är nytt
                        ...
                        else: # namnet från den nya summeringen finns inte med i gamla summeringen
            ...
            else: # (newSummation == '') or ("User's name" not in newSummation) # TEST
...
'''def summarizeHistory():
    ...
    summation = generateResponse(messages, temp=0.2) # Call OpenAI's API to generate a summation

    if '{none}' not in summation:

        summation += "\n(Time & date: "+datetime.datetime.now().strftime('%H:%M %Y-%m-%d')+")" # Inform the AI about time and date

        chatReset() # Reset the memory
        
        print(Style.BRIGHT+Fore.YELLOW+"Summation:\n"+Style.NORMAL+summation)
        
        # Load the JSON file
        with open(longTermMemoryPath, "r") as f:
            data = json.load(f)
            
        # Add the summation to the JSON file
        if "summations" not in data:
            data["summations"] = []
        data["summations"].append(summation)
        ...
        # Add everything from the loaded summations to history
        #if "summations" in data: # TEST
            #for summation in data["summations"]:
                #history.append({"role": "system", "content": "Summation of earlier conversations: "+summation})

    else:

        print(Fore.YELLOW+"Nothing relevant from conversation to save to long-term memory")
...
def answer(prompt, toolAnswer):
    ...
    if '[PRIO]' in toolAnswer: # Prioritize tool answer to the assistant
        ...
        #history.append({"role": "system", "content": showableToolAnswer})
    ...
    if assistantResponse != '': # If there was a response
        ...
        '''
        # This can be used sometime if it is needed
        global totAnswerTokens
        totAnswerTokens = (
            num_tokens_from_messages(messages, kwargs['model']) # prompt
            + num_tokens_from_string(finalResponse, kwargs['model']) # completion
        )
        print("totAnswerTokens:",totAnswerTokens)
        '''

        # This isn't needed anymore since we have a system checking for max tokens
        '''if messageCount >= messageLimit: # Limit history to a certain amount of messages
            speakNprint("Also, we have reached the limit of the conversation.")
            summarizeHistory() # Convert history (short-term-memory) to summation (long-term-memory)
            chatReset()'''
...
def stream_generator(audio_stream, chunk_size=porcupine.frame_length):
    try:
        while True:
...
print("-"*15)
if wakeWordOn and (textInput == False): print("Waiting for wake-word")
elif wakeWordOn == False: print("Waiting for user...")
